...
class LaposteDecoder(Decoder):
    ...
    def decode(self, body, parts, output_format, response):
        """Laposte XML -> Python."""
        ...
        def get_cleaning_date(response, label_cid):
            if response and label_cid:
                data = response.text.split("<%s>" % label_cid)[1]
                content_type = response.headers['Content-Type']
                boundary = content_type.split('boundary="')[1].split('";')[0]
                between_boundaries = data.split("--%s--" % boundary)[0]
        ...
        def generate_label_response(msg):
            ...
            return {
                ...
                "label": {  # main label
                    "name": "label",
                    "data": parts.get(label_cid) or get_cleaning_date(response, label_cid),
                    "type": output_format
                },
                "parcels": [{
                    "id": 1,
